...
class MakePredictionCLITests(unittest.TestCase):
    def _build_pipeline(self) -> WeatherQuantilePipeline:
    ...
    def _run_cli(self, artifact_dir: Path, input_csv: Path, output_csv: Path) -> subprocess.CompletedProcess[str]:
    ...
    def test_make_prediction_cli_handles_single_row_csv(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = self._run_cli(artifact_dir, input_csv, output_csv)

            self.assertEqual(result.returncode, 0, result.stderr)
            pred_df = pd.read_csv(output_csv)
            self.assertEqual(len(pred_df), 1)
            self.assertIn("q_0.050", pred_df.columns)
            self.assertIn("q_0.750", pred_df.columns)
